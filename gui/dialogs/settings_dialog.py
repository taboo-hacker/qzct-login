...
from system_core import global_config, DEFAULT_CONFIG, load_config, save_config
...
class SettingsDialog(QDialog):
    ...
    def _create_wifi_tab(self) -> QWidget:
        ...
        self.wifi_password_edit.setMinimumHeight(38)
        password_layout.addWidget(self.wifi_password_edit)

        self.wifi_password_visible = create_button("显示", btn_type="gray", font_size=10, min_height=34)
        self.wifi_password_visible.setCheckable(True)
        self.wifi_password_visible.setFixedWidth(60)
        self.wifi_password_visible.clicked.connect(
            lambda: self.toggle_password_visibility(self.wifi_password_edit, self.wifi_password_visible)
        )
        password_layout.addWidget(self.wifi_password_visible)
        wifi_layout.addRow("WiFi 密码：", password_layout)
    ...
    def _create_login_tab(self) -> QWidget:
        ...
        self.password_visible = create_button("显示", btn_type="gray", font_size=10, min_height=34)
        self.password_visible.setCheckable(True)
        self.password_visible.setFixedWidth(60)
        self.password_visible.clicked.connect(
            lambda: self.toggle_password_visibility(self.password_edit, self.password_visible)
        )
