...
from gui.styles import FontStyle
...
class CompensatoryWorkdayWidget(QWidget):
    ...
    def init_ui(self) -> None:
        ...
        tip_label = create_tip_label(
            "添加国务院发布的调休补班日期，这些日期即使在节假日期间也需要执行任务"
        )
        ...
        clear_btn = create_button("\U0001f5d1\ufe0f 清空", btn_type="gray", min_width=100)
...
class AddDateDialog(QDialog):
    ...
    def _apply_style(self) -> None:
        self.setStyleSheet(
            StyleManager.get_global_stylesheet() + StyleManager.get_dialog_stylesheet()
        )
