...
本次迭代聚焦**用户使用过程中的流畅度**，而非吞吐性能。实测表明瓶颈不在渲染速度（冷启动 0.79s、1500 行日志渲染 73ms、主题切换 2.7ms/次，均在可接受范围），而在于几处交互细节与两个真实缺陷。所有改动均以回归测试与前后实测数据背书。

---

## 亮点（Top 5）

1. **窗口状态持久化** — 退出时记住窗口大小/位置与所在标签页，下次启动原样还原。此前无任何 `saveGeometry`/`restoreGeometry`，用户每次启动都要重新调整窗口，并总是被带回「运行日志」页。
2. **任务链进度可见** — 状态栏显示「正在执行：连接WiFi（第 2/4 步）」与单步耗时。WiFi 重试最长可达数分钟，此前状态栏一直停在上一步的「完成」文案，用户看不出程序仍在工作。
3. **日志不再打断阅读** — 上滚查阅历史时，新日志不再把视图拽回底部。修复中发现 `setTextCursor` 本身也会滚动视图，仅去掉 `ensureCursorVisible` 并不够，必须同时还原滚动值。
4. **失败提示可操作化** — WiFi/登录/关机失败时直接给出排查入口（如「请到设置核对 WiFi 名称与密码」），状态栏与托盘气泡共用同一映射。
5. **两个真实缺陷修复** — ①重复启动偶尔唤不起已运行实例的窗口（写缓冲未排空即断开，消息被丢弃）；②配置加载补齐 `DATE_RULES` 子键时未深拷贝，会污染进程内默认配置。
...
- 配置项 `ACTIVE_TAB` / `WINDOW_GEOMETRY`（含 schema 校验与值域），标签页标识值域收敛到 `core/constants.py` 的 `TAB_NAMES` 单一数据源
- `TaskChain.step_names` 只读属性，与信号携带的 `task_name` 同源
- `append_preserving_scroll`：日志追加统一入口，按用户滚动位置决定是否跟随

### 缺陷修复

- `utils/single_instance.py`：二次启动等待写缓冲排空由固定 10 轮 `processEvents` 改为墙钟截止。`processEvents` 不推进墙钟时间，事件队列繁忙时轮次被耗尽，随后 `disconnectFromServer` 丢弃尚未写入的数据
- `core/config.py`：补齐 `DATE_RULES` 缺失子键时深拷贝，避免与 `DEFAULT_CONFIG` 共享对象

### 测试与工程

- 修复窗口销毁泄漏：`deleteLater()` 只投递 `DeferredDelete` 事件，而 `processEvents()` 不派发它，导致每个主窗口约 310 个子控件永不释放，后续每次 `setStyleSheet` 都要重新抛光全部存活控件。显式派发后全量测试耗时 **182s → 45s**
- 单实例用例改用「泵事件 + 短睡眠」的截止等待，消除 1/5 的随机失败；新增「事件队列被占满时消息仍应送达」回归用例
- 测试用例 **417 → 448**，覆盖率 **83.26% → 83.63%**；ruff/black/isort/mypy 双模式全程 0 问题
- CI 测试矩阵纳入 **Python 3.14**（2 系统 × 5 版本 = 10 个任务），与最新稳定版保持同步；PySide6 以 `cp310-abi3` wheel 分发，同一份二进制覆盖 3.10+，无需额外构建产物
- 新增 `tools/bench_ux.py` 基准工具（`--out` 存基线、`--compare` 出对比表）

### 已知限制

- 校园网认证仍为 HTTP 明文（网关协议限制，见 README 安全提示）
- 本地构建未做代码签名：当前用户证书库中无 QZCT 代码签名证书，`build.py` 按设计跳过签名（不影响运行，但 Windows 会显示「未知发布者」）
