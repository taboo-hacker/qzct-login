...
- ✅ 农历日历 — 内置农历显示
- ✅ 配置本地存储 — 配置保存在用户目录（`~/.qzct/config.json`），账号密码为明文存储，文件权限已收紧至当前用户
- ✅ 运行日志 — 详细的任务执行记录（登录密码自动脱敏）
- ✅ 简洁商务风界面 — 卡片式两栏布局（状态/操作 + 运行日志），亮色/暗色主题即时切换
- ✅ 键盘快捷键 — Ctrl+R 执行、Ctrl+, 设置页、Ctrl+K 日历页、F1 关于

> ⚠️ **安全提示**：校园网认证协议由网关决定，当前仅支持 HTTP 明文传输，
> 账号密码可能被同网段攻击者截获；请勿与其他系统复用同一密码。
> 配置文件中的密码为明文存储（已通过文件权限收紧缓解），请勿在公共设备上保存。
> WiFi 密码经 netsh 导入后还会以明文保存在 Windows 系统 WLAN profile
> （所有用户级，位于 `C:\ProgramData\Microsoft\Wlan`），是 config.json 之外的
> 第二份明文副本；卸载本程序不会自动清除，可用
> `netsh wlan delete profile name="<SSID>"` 删除，与校园网 HTTP 明文同属已知暴露面。
...
- 🧪 CI 测试矩阵纳入 Python 3.14（3.10–3.14 × Windows/Linux）

### v1.5.3 (2026-08-15)

- ✨ 构建脚本集成代码签名：自动使用 taboo-hacker 个人身份代码签名证书对 exe 签名（SHA256 + 时间戳），随包分发公开证书与一键安装脚本
...
- 🤖 自 v1.4.1 起，项目维护与迭代由 AI 智能体参与（DeepSeek Harness + Agentic Coding）
